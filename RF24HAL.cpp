...
uint32_t RF24HAL::millis() {
	return ST2MS(osalOsGetSystemTimeX());
}

void RF24HAL::delay(uint16_t ms) {
	osalThreadSleepMilliseconds(ms);
}

void RF24HAL::delayMicroseconds(uint16_t us) {
	osalThreadSleepMicroseconds(us);
}

void RF24HAL::csn(int mode)
{
  if(mode)
  {
...
uint8_t RF24HAL::spi_transfer(uint8_t tx) {
	uint8_t rx = 0;
